...
class AdminsLogs extends Component {
  state = {
    logs: []
  }

  componentDidMount() {
    axios.get(api_url+`/logs/admins`)
      .then(res => {
          const logs = res.data;
          this.setState({ logs });
      })
...
//Start RENDER
  ...
  const nr = window.location.href.split('adminslogs/')
  ...
  listItems = <tr><td colSpan='6'>No data OR database connection.</td></tr>
    ...
    //Start Pagination
      ...
      for (var i = 1; i <= total;) {
        i = i + perPage
        pages.push('/#/pages/logs/adminslogs/'+j)
        j++
      }
      var prevPageURLNr = Page-1
      var prevURL = undefined
      prevPageURLNr > 0 ?  prevURL = '/#/pages/logs/adminslogs/'+prevPageURLNr : prevURL = undefined

      var nextPageURLNr = Page+1
      var nextURL = undefined
      nextPageURLNr <= nrPages ?  nextURL = '/#/pages/logs/adminslogs/'+nextPageURLNr : nextURL = undefined
        ...
        <div className="animated fadeIn">
          <Row>
            <Col xs="12" lg="12">
              <Card>
                <CardHeader>
                  <i className="fa fa-align-justify"></i> Admins Logs
                </CardHeader>
                <CardBody>
                  {pageIng()}
                  <Table responsive>
